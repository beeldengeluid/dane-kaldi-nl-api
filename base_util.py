...
def validate_config(cfg, logger):
    ...
    if (
        not cfg["LOG_LEVEL_CONSOLE"] in valid_log_levels
        or not cfg["LOG_LEVEL_FILE"] in valid_log_levels
    ):
...
def init_cache_dir(cfg, logger):
    logger.info(
        "Checking if the PID cache dir {0} exists".format(
            os.path.realpath(cfg["PID_CACHE_DIR"])
        )
    )
...
def validate_data_dirs(cfg, logger):
    ...
    if not os.path.exists(i_dir.parent.absolute()):
        logger.debug(
            "{} does not exist. Make sure BASE_FS_MOUNT_DIR exists before retrying".format(
...
def init_logger(log_dir, log_name, log_level_console, log_level_file):
    ...
    fh = logging.FileHandler(os.path.join(os.path.realpath(log_dir), log_name))
